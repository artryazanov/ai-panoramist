...
**AI Panoramist** 是一个 Python 命令行（CLI）工具，旨在使用 Google Gemini 先进的多模态生成和提示词增强功能，生成无缝的等距柱状投影（equirectangular）360度全景图。

通过自动应用数学上正确的格式标记和空间逻辑（天顶/天底），该工具有助于生成高质量的资产，可直接用于 VR 贴图或 360 度全景查看器。
...
- **提示词增强循环**：将简单的用户提示词（例如：“夜晚的赛博朋克城市”）转换为包含天顶、天底和地平线规格的、符合 VR 标准的严格提示词。
- **自动 QA 验证与完善**：内置的 LLM QA 会检查生成的图像，确保它们看起来像标准的全景图。如果尝试失败，它会将评估意见反馈给生成器，以便在下一次尝试中生成修正后的全景图。
- **条件性无缝融合**：生成成功后，工具会分析图像是否存在垂直后接缝。如果检测到明显的接缝，它会自动使用 AI 图像修复（inpainting）和 Alpha 混合进行修复，确保完美的无缝 360 度环绕。
- **多模态参考支持**：提供本地图像来引导生成过程（风格迁移、角色包含或基础布局）。
- **支持 Docker**：使用 Docker Compose，在隔离环境中轻松部署和运行。

## 🛠️ 前置条件

- **Python 3.12+**（如果在本地运行）
- **Docker** 和 **Docker Compose**（推荐用于环境隔离）
- **Google Cloud API 密钥**，具备访问 Gemini 模型（包含图像生成功能）的权限。
...
如果您想使用 Docker 运行特定的 CLI 命令，可以执行以下命令：

```bash
docker-compose run --rm panoramist --prompt "A sunny beach panorama" --image-refs ./references/sun.jpg
```
*（请确保容器内可以访问所有参考路径，通常可以通过挂载或将它们放置在您的项目目录中来实现）。*

## 🏗️ 架构

该项目在 `app/core` 目录下主要包含三个组件：
1. **PromptEnhancer（提示词增强器）**：与文本模型（`TEXT_MODEL_NAME`）通信，将简短的用户描述扩展为详细的 VR 提示词。
2. **GenAIClient（生成式 AI 客户端）**：封装 `google-genai` SDK，用于处理结构化输出解析、多模态图像生成（`IMAGE_MODEL_NAME`）以及 QA 视觉检查（`VALIDATOR_MODEL_NAME`）。
3. **Panoramist（全景协调器）**：协调器模块，将提示词增强、图像生成和验证重试绑定成一个连贯的循环。
